...
public class PaginationConfig {
  ...
  /**
   * Tri appliqué aux journaux d'audit paginés quand la requête ne fournit pas
   * son propre paramètre {@code sort} : du plus récent au plus ancien.
   */
  @Bean
  // Definit le tri par defaut des requetes paginees.
  public SortHandlerMethodArgumentResolverCustomizer defaultSortCustomizer() {
    return resolver ->
      resolver.setFallbackSort(Sort.by(Sort.Direction.DESC, "timestamp"));
  }
}
